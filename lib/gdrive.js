 ...
 *
 * uploadFileBuffer() and listFolderChildren() accept an optional folderId so the
 * caller can route each class to its own folder; if omitted they fall back to
 * GDRIVE_FOLDER_ID.
 */
...
// Fallback folder when the caller doesn't pass one explicitly.
...
async function uploadFileBuffer(fileName, buffer, contentType, targetFolderId) {
  const drive = getDrive();
  const parentId = targetFolderId || defaultFolderId();
...
async function listFolderChildren(pageSize = 50, targetFolderId) {
  const drive = getDrive();
  const parentId = targetFolderId || defaultFolderId();
